 ...
 return (
  ...
  >
   <a href={liveUrl} target="_blank">
    <article
     className={cn(
      "relative h-full max-w-[42rem] overflow-hidden rounded-3xl border border-black/5 bg-gray-100 p-4 transition dark:bg-[#292f3d] dark:text-white sm:p-8",
      liveUrl ? "hover:bg-gray-200 dark:hover:bg-white/20" : "",
     )}
    >
     <div className="flex h-full flex-col">
      <div className="flex items-start justify-between gap-4">
       <h3 className="font-lmroman text-2xl">{title}</h3>
       {liveUrl && <HiOutlineExternalLink className="size-5" />}
      </div>
      <p className="mt-2 text-[15px] leading-relaxed text-gray-700 dark:text-white/70">{description}</p>
      <ul className="mt-4 flex flex-wrap gap-2">
       {tags.map((tag, index) => (
        <li
         className="rounded-full bg-[#4a4b4b] px-3 py-1 text-[0.7rem] uppercase tracking-wider text-white dark:bg-gray-700 dark:text-white/70"
         key={index}
        >
         {tag}
        </li>
